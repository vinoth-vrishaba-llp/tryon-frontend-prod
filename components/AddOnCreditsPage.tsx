        ...
        <div className="min-h-screen bg-surface-secondary py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-5xl mx-auto">
                <div className="flex items-center justify-between mb-12">
                    <div>
                        <h1 className="text-4xl font-black text-content tracking-tight">Add-On Credits</h1>
                        <p className="mt-2 text-lg text-content-secondary">Top up your credits to keep creating stunning renders.</p>
                    </div>
                    <button
                        onClick={onBack}
                ...
                {!canPurchase && (
                    <div className="mb-8 p-6 bg-surface-tertiary border-2 border-border-strong rounded-2xl animate-in fade-in slide-in-from-top-4">
                        <div className="flex items-start gap-4">
                            <svg className="w-8 h-8 text-content flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                            </svg>
                            <div className="flex-1">
                                <h3 className="text-lg font-black text-content mb-2">Access Restricted</h3>
                                <p className="text-content-secondary font-bold mb-4">
                                ...
                                <button
                                    ...
                                    className="px-6 py-3 bg-primary text-content-inverse font-black rounded-xl hover:bg-interactive-hover transition-colors"
                                >
                                    View Plans & Pricing
                                </button>
                            </div>
                ...
                {message && (
                    <div className={`mb-8 p-4 rounded-2xl border ${
                        message.type === 'success' ? 'bg-surface-secondary border-border text-primary' :
                        message.type === 'warning' ? 'bg-surface-tertiary border-border-secondary text-content-secondary' :
                        'bg-surface-tertiary border-border text-content'
                ...
                <div className={`grid grid-cols-1 md:grid-cols-3 gap-8 ${!canPurchase ? 'opacity-50 pointer-events-none' : ''}`}>
                    {CREDIT_PACKS.map((pack) => (
                        <div
                            key={pack.id}
                            className={`relative bg-surface rounded-3xl p-8 shadow-xl border-2 transition-all hover:scale-105 ${
                                pack.popular ? 'border-border-strong' : 'border-transparent'
                            }`}
                        >
                            {pack.popular && (
                                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-primary text-content-inverse px-4 py-1 rounded-full text-xs font-black uppercase tracking-widest shadow-lg">
                                    Most Popular
                                </div>
                            )}

                            <div className="mb-6">
                                <h3 className="text-xl font-black text-content mb-2">{pack.name}</h3>
                                <p className="text-content-tertiary text-sm leading-relaxed">{pack.description}</p>
                            </div>

                            <div className="mb-8">
                                <div className="flex items-baseline gap-1">
                                    <span className="text-5xl font-black text-content">{pack.credits}</span>
                                    <span className="text-xl font-bold text-content-disabled">⚡ Credits</span>
                                </div>
                            ...
                            <button
                                ...
                                className={`w-full py-4 rounded-2xl font-black text-lg transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed ${
                                    pack.popular
                                        ? 'bg-primary text-content-inverse hover:bg-interactive-hover shadow-xl'
                                        : 'bg-primary text-content-inverse hover:bg-interactive-hover'
                                }`}
                            >
                ...
                <div className="mt-12 bg-surface-tertiary rounded-3xl p-8 border border-border">
                    <div className="flex items-start gap-4">
                        <div className="bg-surface-secondary p-3 rounded-2xl">
                            <svg className="w-6 h-6 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                        </div>
                        <div>
                            <h4 className="text-lg font-bold text-content mb-1">About Add-On Credits</h4>
                            ...
                            <p className="text-content-tertiary text-xs font-bold">
