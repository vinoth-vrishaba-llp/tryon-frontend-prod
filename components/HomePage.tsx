...
export const GenerationProgress: React.FC<GenerationProgressProps> = ({ stage = GenerationStage.INITIALIZING }) => {
  ...
  return (
    <div className="w-full max-w-lg mx-auto p-10 bg-white rounded-2xl shadow-2xl border border-gray-100 relative overflow-hidden">
      ...
      <div className="flex flex-col items-center mb-8">
        <div className="relative mb-6">
          ...
          <div className="relative z-10 bg-primary p-5 rounded-full shadow-xl">
            <svg className="h-12 w-12 text-white animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
          </div>
        </div>
        <h3 className="text-2xl font-black text-gray-800 tracking-tight text-center">{STAGE_LABELS[stage]}</h3>
        <p className="text-sm text-secondary font-medium mt-2 animate-pulse">~{Math.round(progress)}% Processed</p>
      </div>

      <div className="relative w-full bg-gray-100 rounded-full h-4 overflow-hidden mb-2">
        <div className="h-full bg-primary transition-all duration-500" style={{ width: `${progress}%` }}></div>
      </div>
      <p className="text-center text-[10px] text-gray-400 font-bold uppercase tracking-widest">Digital Loom in operation...</p>
    </div>
  );
};
...
const CategoryCard: React.FC<{ title: string; description: string; imageUrl: string; onClick: () => void }> = ({ 
  ...
  >
    ...
    />
    <div className="absolute inset-0 bg-gradient-to-t from-primary/95 via-primary/20 to-transparent flex flex-col items-center justify-end p-6 text-center">
      <h3 className="text-2xl font-black text-white mb-2">{title}</h3>
      <p className="text-xs text-gray-200 font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-500">{description}</p>
    </div>
  </div>
);
